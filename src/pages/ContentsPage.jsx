...
const ContentsPage = () => {
  ...
  // userRole es un número, no un objeto
  ...
  // Usuarios básicos, gestores y administradores solo ven "Programados"
  ...
  logger.dev('🔍 ContentsPage - userRole:', userRole, 'isBasicUser:', isBasicUser, 'isAdmin:', isAdmin, 'shouldShowOnlyScheduled:', shouldShowOnlyScheduled);
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 pb-24">
      ...
      <Tabs defaultValue={shouldShowOnlyScheduled ? "scheduled" : "recents"} className="w-full">
        {/* Usuarios básicos y administradores solo ven "Programados" */}
        {shouldShowOnlyScheduled ? (
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (authUserId) {
                        ...
                        window.open('/gestor?upgrade=true', '_blank');
                      }
                    } catch (err) {
          ...
          <TabsList className={`grid w-full ${hasPermission('canAccessDrafts') ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4' : 'grid-cols-2 sm:grid-cols-3'} gap-2 mb-6 bg-card/80 p-1.5 rounded-lg border border-border/70`}>
            <TabsTrigger value="recents" className="data-[state=active]:bg-[#A2D9F7]/20 data-[state=active]:text-[#A2D9F7]">Recientes</TabsTrigger>
            <TabsTrigger value="scheduled" className="data-[state=active]:bg-[#A2D9F7]/20 data-[state=active]:text-[#A2D9F7]">Programados</TabsTrigger>
            <PermissionGated permissions={['canAccessDrafts']}>
              <TabsTrigger value="drafts" className="data-[state=active]:bg-[#A2D9F7]/20 data-[state=active]:text-[#A2D9F7]">Borradores</TabsTrigger>
            </PermissionGated>
                ...
                <CardHeader>
                  ...
                  {isAISearchActive && searchTerm && (
                    <motion.div 
                      ...
                      className="mt-2.5 text-xs text-[#A2D9F7] flex items-center bg-[#A2D9F7]/10 px-2 py-1 rounded-md border border-[#A2D9F7]/20">
                      <Brain size={14} className="mr-1.5 text-[#A2D9F7] animate-pulse" style={{animationDuration: '1.5s'}} />
                ...
                <CardContent>
                  {filteredRecentAds.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                      {filteredRecentAds.map((ad, i) => (
                        <motion.div
                          ...
                          whileHover="hover"
                        >
                          <Card className={`overflow-hidden h-full flex flex-col border-l-4 ${ad.color.split(' ')[2]} bg-card/80`}>
                            ...
                            <CardFooter className="py-2.5 px-4 bg-muted/20 dark:bg-muted/10 border-t border-border/60 flex justify-end space-x-1.5">
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-[#A2D9F7] hover:bg-[#A2D9F7]/10"> <PlayCircle size={16} /> </Button>
                    ...
                    <div className="text-center py-10">
                      <Archive size={48} className="mx-auto text-muted-foreground/50 mb-4 animate-pulse" style={{ animationDuration: '4s' }} />
            ...
            <PermissionGated permissions={['canAccessDrafts']}>
              <TabsContent value="drafts">
                <div className="text-center py-16">
                  <Edit3 size={56} className="mx-auto text-muted-foreground/40 mb-5 animate-pulse" style={{ animationDuration: '3s' }} />
        ...
        {!shouldShowOnlyScheduled && (
          <TabsContent value="library">
            <div className="text-center py-16">
              ...
              <h3 className="text-2xl text-foreground font-sans">Biblioteca de Contenidos</h3>
              <p className="text-muted-foreground text-sm mt-1 font-sans">Esta sección estará disponible próximamente para gestionar tu biblioteca de contenidos.</p>
            </div>
          </TabsContent>
        )}
      </Tabs>
